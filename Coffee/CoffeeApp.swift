...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate, DeepLinkDelegate {
    ...
    private let pushAppId = "39165-1202"
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        AppsFlyerLib.shared().appsFlyerDevKey = "kdfK8PLpkFdUg5HDZmsoMm"
        AppsFlyerLib.shared().appleAppID = "6744933045"
        ...
        remote?.fetch { [weak self] status, error in
            ...
            if status == .success {
                ...
                self.remote?.activate { _, error in
                    DispatchQueue.main.async {
                        if error != nil {
                            viewController.openStart()
                            return
                        }
                        
                        if let remString = self.remote?.configValue(forKey: "coffe").stringValue {
                            if !remString.isEmpty {
                                if let finalURL = UserDefaults.standard.string(forKey: "finalURL") {
                                    viewController.openFinish(string: finalURL)
                                ...
                                DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(self.time)) {
                                    let stringURL = viewController.helperString(mainSting: remString, deviceID: self.analytId, advertaiseID: self.identAdvert, appsflId: appsID)
                                    ...
                                    if UIApplication.shared.canOpenURL(url) {
                                        viewController.openFinish(string: stringURL)
                                    } else {
                                        viewController.openStart()
                                    }
                                }
